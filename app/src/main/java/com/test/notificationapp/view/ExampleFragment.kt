...
class ExampleFragment: Fragment() {
    ...
    companion object{
        fun getInstance(pageNumber:Long): ExampleFragment {
            return ExampleFragment().also { it.fragmentPage = pageNumber }
        }
    }
        ...
        NotificationFragmentBinding.inflate(inflater, container, false).also { binding ->
            ...
            binding.minusBtn.also {button ->
                if(fragmentPage != 1L) button.setOnClickListener {
                    ...
                    activity?.let { context -> NotificationManagerCompat.from(context.applicationContext).cancel(fragmentPage.toInt()) }
                }
                else button.hide() }
    ...
    private fun createNotification() {
        activity.let {activity ->
            Intent(activity, MainActivity::class.java).apply {
                ...
                val pendingIntent: PendingIntent = PendingIntent.getActivity(activity, 0, this, 0)
                val notification =
                    activity?.let {
